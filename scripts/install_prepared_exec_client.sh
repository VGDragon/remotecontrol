...
KEY_SERVER_NAME=${KEY_SERVER_NAME%.*}
echo $KEY_SERVER_NAME


TARGET_DIR="/apps/remotecontrol"
TARGET_EXECUTE_CLIENT_DIR="${TARGET_DIR}/execute_client"
KEYPAIRS_DIR="${TARGET_EXECUTE_CLIENT_DIR}/keypairs"
...
sed -i 's/"isServer":true/"isServer":false/g' "${TARGET_EXECUTE_CLIENT_DIR}/data.json"
sed -i 's/"isClient":false/"isClient":true/g' "${TARGET_EXECUTE_CLIENT_DIR}/data.json"
sed -i 's/"exec":false/"exec":true/g' "${TARGET_EXECUTE_CLIENT_DIR}/data.json"


cp "${KEY_TO_BE_USED}" "${KEYPAIRS_DIR}/${KEY_SERVER_NAME}"
sed -i 's/"computerName":""/"computerName":"'"${KEY_CLIENT_NAME}"'"/g' "${TARGET_EXECUTE_CLIENT_DIR}/data.json"
